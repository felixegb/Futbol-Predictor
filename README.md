...
normalmente un ejecutable de la `.venv` quedo apuntando a una ruta vieja. Solucion rapida:

```powershell
python -m pip install --force-reinstall uvicorn
```

Si el problema persiste con varios paquetes, recrea la `.venv` desde cero e instala nuevamente `requirements.txt`.

## Licencia

Puedes usar este proyecto como base para aprendizaje y portfolio. Ajusta esta seccion segun la licencia que quieras publicar en GitHub (por ejemplo MIT).
